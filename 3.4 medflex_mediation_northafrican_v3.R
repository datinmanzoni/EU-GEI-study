...
data_northafrican <- read.csv("missRanger_postimputed_data_2025_v3.csv")
...
table(data_northafrican$parentssocialclass)
str(data_northafrican$parentssocialclass)
...
table(data_northafrican2$subjectstatus)
...
table(data_northafrican2$parentssocialclass)
str(data_northafrican2$parentssocialclass)

str(data_northafrican2$dfar.all)
str(data_northafrican2$benton.total)

# Save filtered northafrican dataset
write.csv(data_northafrican2,"data_northafrican_filtered_2024_v3.csv")
...
### Step 1: Fit an Outcome Regression Model (mediator model)
Mi2fit <-glm(
  formula = dfar.all ~ factor(group_northafrican) + age + gender + country + parentssocialclass + benton.total,
  data = data_northafrican2,
  family= gaussian()
)
...
neMod2 <- neModel(
  formula = subjectstatus ~ group_northafrican0 + group_northafrican1 + age + gender + country + parentssocialclass + benton.total,
  family = binomial(link = "logit"), 
  expData = mi2data, 
  se = "bootstrap", 
  nBoot = 1000,
...
### Step 1: Fit an Outcome Regression Model (mediator model)
Mi2fit_2 <-glm(
  formula = negativity.bias ~ factor(group_northafrican) + age + gender + country + parentssocialclass + benton.total,
  data = data_northafrican2,
  family= gaussian()
)

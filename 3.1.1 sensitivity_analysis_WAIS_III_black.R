...
str(data_black2$symbol_coding_scaled)

# Save filtered black dataset
write.csv(data_black2,"data_black_filtered_sensitivity_analysis.csv")
...
### Step 1: Fit an Outcome Regression Model (mediator model)
Mi2fit <-glm(
  formula = dfar.all ~ factor(group_black) + age + gender + country + symbol_coding_scaled + benton.total,
  data = data_black2,
  family= gaussian()
)
...
neMod2 <- neModel(
  formula = subjectstatus ~ group_black0 + group_black1 + age + gender + country + symbol_coding_scaled + benton.total,
  family = binomial(link = "logit"), 
  expData = mi2data, 
  se = "bootstrap", 
  nBoot = 1000,
...
### Step 1: Fit an Outcome Regression Model (mediator model)
Mi2fit_2 <-glm(
  formula = negativity.bias ~ factor(group_black) + age + gender + country + symbol_coding_scaled + benton.total,
  data = data_black2,
  family= gaussian()
)

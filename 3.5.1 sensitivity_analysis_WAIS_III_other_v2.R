...
data_other <- read.csv("missRanger_postimputed_data_2025_sensitivity_analysis_IQ.csv")
...
str(data_other2$symbol_coding_scaled)

# Save filtered other dataset
write.csv(data_other2,"data_other_filtered_sensitivity_analysis_v2.csv")
...
### Step 1: Fit an Outcome Regression Model (mediator model)
Mi2fit <-glm(
  formula = dfar.all ~ factor(group_other) + age + gender + country + parentssocialclass + symbol_coding_scaled + benton.total,
  data = data_other2,
  family= gaussian()
)
...
neMod2 <- neModel(
  formula = subjectstatus ~ group_other0 + group_other1 + age + gender + country + parentssocialclass + symbol_coding_scaled + benton.total,
  family = binomial(link = "logit"), 
  expData = mi2data, 
  se = "bootstrap", 
  nBoot = 1000,
...
### Step 1: Fit an Outcome Regression Model (mediator model)
Mi2fit_2 <-glm(
  formula = negativity.bias ~ factor(group_other) + age + gender + country + parentssocialclass + symbol_coding_scaled + benton.total,
  data = data_other2,
  family= gaussian()
)

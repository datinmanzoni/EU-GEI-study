...
data_mixed <- read.csv("missRanger_postimputed_data_2025_v3.csv")
...
table(data_mixed$parentssocialclass)
str(data_mixed$parentssocialclass)
...
table(data_mixed2$subjectstatus)
...
table(data_mixed2$parentssocialclass)
str(data_mixed2$parentssocialclass)

str(data_mixed2$dfar.all)
str(data_mixed2$benton.total)

# Save filtered mixed dataset
write.csv(data_mixed2,"data_mixed_filtered_2024_v3.csv")
...
### Step 1: Fit an Outcome Regression Model (mediator model)
Mi2fit <-glm(
  formula = dfar.all ~ factor(group_mixed) + age + gender + country + parentssocialclass + benton.total,
  data = data_mixed2,
  family= gaussian()
)
...
neMod2 <- neModel(
  formula = subjectstatus ~ group_mixed0 + group_mixed1 + age + gender + country + parentssocialclass + benton.total,
  family = binomial(link = "logit"), 
  expData = mi2data, 
  se = "bootstrap", 
  nBoot = 1000,
...
### Step 1: Fit an Outcome Regression Model (mediator model)
Mi2fit_2 <-glm(
  formula = negativity.bias ~ factor(group_mixed) + age + gender + country + parentssocialclass + benton.total,
  data = data_mixed2,
  family= gaussian()
)

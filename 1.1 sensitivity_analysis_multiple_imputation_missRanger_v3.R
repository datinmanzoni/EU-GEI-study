...
data_preimp$block_design_scaled_EU <- data_preimp$blokgs
data_preimp$block_design_scaled_BR <- data_preimp$blockscaled
data_preimp$information_scaled_EU <- data_preimp$informgs
data_preimp$information_scaled_BR <- data_preimp$informscaled
data_preimp$arithmet_scaled_EU <- data_preimp$rekgs
data_preimp$arithmet_scaled_BR <- data_preimp$arithmscaled
...
table(is.na(data_preimp$block_design_scaled_EU))
table(is.na(data_preimp$block_design_scaled_BR))
table(is.na(data_preimp$information_scaled_EU))
table(is.na(data_preimp$information_scaled_BR))
table(is.na(data_preimp$arithmet_scaled_EU))
table(is.na(data_preimp$arithmet_scaled_BR))
...
data_preimp$block_design_scaled <- coalesce(data_preimp$block_design_scaled_EU, data_preimp$block_design_scaled_BR)
data_preimp$information_scaled <- coalesce(data_preimp$information_scaled_EU, data_preimp$information_scaled_BR)
data_preimp$arithmet_scaled <- coalesce(data_preimp$arithmet_scaled_EU, data_preimp$arithmet_scaled_BR)

# Check execution
sum(!is.na(data_preimp$IQ_total))
sum(!is.na(data_preimp$symbol_coding_scaled))
sum(!is.na(data_preimp$block_design_scaled))
sum(!is.na(data_preimp$information_scaled))
sum(!is.na(data_preimp$arithmet_scaled))
...
data_preimp_filtered2 <- subset(data_preimp, select = c(subjectID, subjectstatus, ethnicity, age, gender, ethnicitysitespecific, 
                                                       ...
                                                       p.fearhappy, p.fearneutral, p.neutralanger, p.neutralfear, p.neutralhappy, IQ_total, 
                                                       symbol_coding_scaled, block_design_scaled, information_scaled, arithmet_scaled))
...
table((is.na(data_preimp_filtered2$block_design_scaled)),  data_preimp_filtered2$subjectstatus)
table((is.na(data_preimp_filtered2$information_scaled)),  data_preimp_filtered2$subjectstatus)
table((is.na(data_preimp_filtered2$arithmet_scaled)), data_preimp_filtered2$subjectstatus)
...
wilcox.blkdsgn <- wilcox.test(block_design_scaled ~ subjectstatus, data = data_imputed)
wilcox.info <- wilcox.test(information_scaled ~ subjectstatus, data = data_imputed)
wilcox.arithm <- wilcox.test(arithmet_scaled ~ subjectstatus, data = data_imputed)
...
W.blkdsgn <- wilcox.blkdsgn$statistic 
W.info <- wilcox.info$statistic 
W.arithm <- wilcox.arithm$statistic 
...
Z.blkdsgn <- (W.blkdsgn - mu_W) / sigma_W
Z.info <- (W.info - mu_W) / sigma_W
Z.arithm <- (W.arithm - mu_W) / sigma_W
...
wilcox.test(block_design_scaled ~ subjectstatus, data = data_imputed)
wilcox.test(information_scaled ~ subjectstatus, data = data_imputed)
wilcox.test(arithmet_scaled ~ subjectstatus, data = data_imputed)

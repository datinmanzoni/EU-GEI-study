...
data_other <- read.csv("missRanger_postimputed_data_2025_v3.csv")

table(data_other$subjectstatus)
str(data_other$subjectstatus)
table(data_other$gender)
str(data_other$gender)
table(data_other$country)
str(data_other$country)
table(data_other$parentssocialclass)
str(data_other$parentssocialclass)
table(data_other$ethnicity)
str(data_other$ethnicity)
...
# Filter the dataset for "white" and "other" subjects
data_other2 <- data_other %>%
  filter(ethnicity %in% c("white", "other"))

# Dummy coding 'group_other' variable: "white" = 0, "other" = 1
data_other2 <- data_other2 %>%
  mutate(group_other = ifelse(ethnicity == "other", 1, 0))

# Reorder columns
data_other2 <- data_other2 %>%
  relocate(group_other, .after = ethnicity)

# Checks
str(data_other2$group_other)
table(data_other2$group_other)

# Recode group_other into factor 
data_other2 <- data_other2 %>%
  mutate(group_other = factor(group_other, levels = c(0, 1)))

# Checks
str(data_other2$group_other)
table(data_other2$group_other)
levels(data_other2$group_other)

table(data_other2$subjectstatus, data_other2$group_other)

table(data_other2$subjectstatus)
str(data_other2$subjectstatus)

table(data_other2$gender)
str(data_other2$gender)

table(data_other2$country)
str(data_other2$country)

table(data_other2$parentssocialclass)
str(data_other2$parentssocialclass)

str(data_other2$dfar.all)
str(data_other2$benton.total)

# Save filtered other dataset
write.csv(data_other2,"data_other_filtered_2024_v3.csv")
...
Mi0fit <-glm(
  formula = dfar.all ~ factor(group_other),
  data = data_other2,
  family= gaussian()
)
...
neMod0 <- neModel(
  formula = subjectstatus ~ group_other0 + group_other1,
...
### Step 1: Fit an Outcome Regression Model (mediator model)
Mi1fit <-glm(
  formula = dfar.all ~ factor(group_other) + age + gender + country,
  data = data_other2,
  family= gaussian()
)
...
neMod1 <- neModel(
  formula = subjectstatus ~ group_other0 + group_other1 + age + gender + country,
  family = binomial(link = "logit"), 
  expData = mi1data, 
  se = "bootstrap", 
  nBoot = 1000,
...
### Step 1: Fit an Outcome Regression Model (mediator model)
Mi2fit <-glm(
  formula = dfar.all ~ factor(group_other) + age + gender + country + parentssocialclass + benton.total,
  data = data_other2,
  family= gaussian()
)
...
neMod2 <- neModel(
  formula = subjectstatus ~ group_other0 + group_other1 + age + gender + country + parentssocialclass + benton.total,
  family = binomial(link = "logit"), 
  expData = mi2data, 
  se = "bootstrap", 
  nBoot = 1000,
...
### Step 1: Fit an Outcome Regression Model (mediator model)
Mi0fit_2 <-glm(
  formula = negativity.bias ~ factor(group_other),
  data = data_other2,
  family= gaussian()
)
...
### Step 1: Fit an Outcome Regression Model (mediator model)
Mi1fit_2 <-glm(
  formula = negativity.bias ~ factor(group_other) + age + gender + country,
  data = data_other2,
  family= gaussian()
)
...
### Step 1: Fit an Outcome Regression Model (mediator model)
Mi2fit_2 <-glm(
  formula = negativity.bias ~ factor(group_other) + age + gender + country + parentssocialclass + benton.total,
  data = data_other2,
  family= gaussian()
)

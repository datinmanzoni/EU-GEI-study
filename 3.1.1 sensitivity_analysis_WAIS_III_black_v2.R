...
data_black <- read.csv("missRanger_postimputed_data_2025_sensitivity_analysis_IQ.csv")
...
write.csv(data_black2,"data_black_filtered_sensitivity_analysis_v2.csv")
...
### Step 1: Fit an Outcome Regression Model (mediator model)
Mi2fit <-glm(
  formula = dfar.all ~ factor(group_black) + age + gender + country + parentssocialclass + symbol_coding_scaled + benton.total,
  data = data_black2,
  family= gaussian()
)
...
neMod2 <- neModel(
  formula = subjectstatus ~ group_black0 + group_black1 + age + gender + country + parentssocialclass + symbol_coding_scaled + benton.total,
  family = binomial(link = "logit"), 
  expData = mi2data, 
  se = "bootstrap", 
  nBoot = 1000,
...
### Step 1: Fit an Outcome Regression Model (mediator model)
Mi2fit_2 <-glm(
  formula = negativity.bias ~ factor(group_black) + age + gender + country + parentssocialclass + symbol_coding_scaled + benton.total,
  data = data_black2,
  family= gaussian()
)

...
write.csv(data_northafrican2,"data_northafrican_filtered_sensitivity_analysis.csv")
...
### Step 1: Fit an Outcome Regression Model (mediator model)
Mi2fit <-glm(
  formula = dfar.all ~ factor(group_northafrican) + age + gender + country + symbol_coding_scaled + benton.total,
  data = data_northafrican2,
  family= gaussian()
)
...
neMod2 <- neModel(
  formula = subjectstatus ~ group_northafrican0 + group_northafrican1 + age + gender + country + symbol_coding_scaled + benton.total,
  family = binomial(link = "logit"), 
  expData = mi2data, 
  se = "bootstrap", 
  nBoot = 1000,
...
### Step 1: Fit an Outcome Regression Model (mediator model)
Mi2fit_2 <-glm(
  formula = negativity.bias ~ factor(group_northafrican) + age + gender + country + symbol_coding_scaled + benton.total,
  data = data_northafrican2,
  family= gaussian()
)

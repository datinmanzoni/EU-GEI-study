...
### Step 1: Fit an Outcome Regression Model (mediator model)
Mi2fit <-glm(
  formula = dfar.all ~ factor(group_mixed) + age + gender + country + parentssocialclass + symbol_coding_scaled + benton.total,
  data = data_mixed2,
  family= gaussian()
)
...
neMod2 <- neModel(
  formula = subjectstatus ~ group_mixed0 + group_mixed1 + age + gender + country + parentssocialclass + symbol_coding_scaled + benton.total,
  family = binomial(link = "logit"), 
  expData = mi2data, 
  se = "bootstrap", 
  nBoot = 1000,
...
## Model 2: age, gender, country, parental social classs, processing speed, general facial recognition

### Step 1: Fit an Outcome Regression Model (mediator model)
Mi2fit_2 <-glm(
  formula = negativity.bias ~ factor(group_mixed) + age + gender + country + parentssocialclass + symbol_coding_scaled + benton.total,
  data = data_mixed2,
  family= gaussian()
)

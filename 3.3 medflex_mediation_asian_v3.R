...
data_asian <- read.csv("missRanger_postimputed_data_2025_v3.csv")
...
table(data_asian$parentssocialclass)
str(data_asian$parentssocialclass)
...
table(data_asian2$subjectstatus)
...
table(data_asian2$parentssocialclass)
str(data_asian2$parentssocialclass)

str(data_asian2$dfar.all)
str(data_asian2$benton.total)

# Save filtered asian dataset
write.csv(data_asian2,"data_asian_filtered_2024_v3.csv")
...
### Step 1: Fit an Outcome Regression Model (mediator model)
Mi2fit <-glm(
  formula = dfar.all ~ factor(group_asian) + age + gender + country + parentssocialclass + benton.total,
  data = data_asian2,
  family= gaussian()
)
...
neMod2 <- neModel(
  formula = subjectstatus ~ group_asian0 + group_asian1 + age + gender + country + parentssocialclass + benton.total,
  family = binomial(link = "logit"), 
  expData = mi2data, 
  se = "bootstrap", 
  nBoot = 1000,
...
### Step 1: Fit an Outcome Regression Model (mediator model)
Mi2fit_2 <-glm(
  formula = negativity.bias ~ factor(group_asian) + age + gender + country + parentssocialclass + benton.total,
  data = data_asian2,
  family= gaussian()
)
